...
int main(int argc, char* argv[]) {
   ...
   double kdivMplred  = static_cast<double>(atof(argv[15]));
...
if( (run_channel3 || run_channel4 ) && (run_channel1 || run_channel2) ){
   if (run_channel1){
      myPixie->SetupFor7and8Combination_RS(myConfigurator->getCombinedWS(),"dimuon2011");
   }
   if (run_channel2){
      myPixie->SetupFor7and8Combination_RS(myConfigurator->getCombinedWS(),"dielectron2011");
   }
}
...
//adjust signal resolution for the RS model

myPixie->SetResolution_RS(myConfigurator->getCombinedWS(), kdivMplred, myConfigurator->getChannelNames());
